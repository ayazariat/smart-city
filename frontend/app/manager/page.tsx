...
// Filter complaints based on all filters
  ...
  const municipality = user?.municipalityName || (typeof user?.municipality === "object" ? user?.municipality?.name : "") || "";
            ...
            <div className="bg-white rounded-2xl shadow-lg p-5 border border-slate-200">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-orange-600 mt-1">{inProgressCount}</p>
                </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="mb-6">
          ...
          {overdueCount > 0 && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-4 flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-red-800">{overdueCount} complaint{overdueCount > 1 ? 's' : ''} overdue</p>
                <p className="text-sm text-red-600">SLA deadline has passed for {overdueCount} complaint{overdueCount > 1 ? 's' : ''}. Take immediate action.</p>
              </div>
            </div>
          )}
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
              <h3 className="text-sm font-semibold text-slate-700 mb-4">Resolution Time Trend</h3>
              <div className="h-32 flex items-end gap-2">
                {[...Array(7)].map((_, i) => (
                  <div key={i} className="flex-1 bg-gradient-to-t from-primary to-primary-500 rounded-t" style={{ height: `${30 + Math.random() * 50}%` }} />
                ))}
              </div>
            </div>
          </div>
