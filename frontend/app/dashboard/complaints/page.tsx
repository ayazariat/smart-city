...
function DashboardContent() {
  ...
  const fetchMunicipalityComplaints = async () => {
    ...
    try {
      ...
      if (data.success && data.complaints && Array.isArray(data.complaints)) {
        complaintsArray = data.complaints;
      } else if (data.success && data.data && Array.isArray(data.data)) {
        complaintsArray = data.data;
      } else if (data.success && data.data && data.data.complaints && Array.isArray(data.data.complaints)) {
        complaintsArray = data.data.complaints;
      ...
      if (complaintsArray) {
        setMunicipalityComplaints(complaintsArray as MunicipalityComplaint[]);
      } else {
        console.error('Could not extract complaints array from response');
        setMunicipalityComplaints([]);
      }
    } catch (err) {
                  ...
                  <div className="p-4 bg-gradient-to-r from-violet-50 to-purple-50 rounded-xl border border-violet-200">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-semibold text-violet-800">
                        {t('stats.avgProcessingTime', {
                          defaultValue: 'Avg. Processing Time',
                        })}
                      </span>
                      <span className="text-lg font-bold text-violet-700">
                        {typeof stats.avgDays === 'number'
                          ? `${stats.avgDays}d`
                    ...
                    <p className="text-xs text-violet-500">
                      {t('stats.avgProcessingDesc', {
                        defaultValue: 'From submission to closure',
                      })}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="relative h-28 bg-gradient-to-br from-green-50 to-slate-50">
                    ...
                    <div className="absolute top-2 right-2">
                      <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-green-100 text-green-700 shadow-sm">
                        {t(
                          `status.${complaint.status === 'CLOSED' ? 'CLOSED' : 'RESOLVED'}`
                        )}
                      </span>
                    </div>
                  </div>
