  ...
  const fetchMunicipalityComplaints = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/public/my-municipality-complaints?limit=20&status=VALIDATED,ASSIGNED,IN_PROGRESS,RESOLVED,CLOSED`,
        { 
          credentials: "include",
          headers: { 
  ...
  return (
    <DashboardLayout>
      <main className="px-4 md:px-6 py-6 md:py-8 max-w-6xl">
          ...
          <div className="bg-gradient-to-br from-white to-primary/5 rounded-2xl shadow-lg p-6 border border-primary/10 mb-8">
            ...
            <div className="space-y-3">
              ...
              {user?.role === "DEPARTMENT_MANAGER" && (
                <>
                  {(stats.resolved || 0) > 0 && (
                    <Link href="/manager/pending?status=RESOLVED" className="flex items-center justify-between p-3 bg-amber-50 rounded-xl border border-amber-200 hover:bg-amber-100 transition-colors shadow-sm">
                      <span className="text-sm font-medium text-amber-700">{stats.resolved} {t('priorities.resolutionsAwaiting')}</span>
                      <ArrowRight className="w-4 h-4 text-amber-500" />
                    </Link>
                  )}
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
                ...
                <div className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl p-4 border border-slate-200">
                  ...
                  <div className="text-xs text-slate-500 mt-1">cases</div>
                </div>
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                ...
                <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4 border border-green-200">
                  ...
                  <div className="text-xs text-green-500 mt-1">cases</div>
                </div>
                <div className={`bg-gradient-to-br ${(stats.totalOverdue || 0) > 0 ? 'from-red-50 to-red-100 border-red-200' : 'from-slate-50 to-slate-100 border-slate-200'} rounded-xl p-4 border`}>
                ...
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-gradient-to-r from-violet-50 to-purple-50 rounded-xl border border-violet-200">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-semibold text-violet-800">{t('stats.avgProcessingTime', { defaultValue: 'Avg. Processing Time' })}</span>
                      <span className="text-lg font-bold text-violet-700">
                        {typeof stats.avgDays === 'number' ? `${stats.avgDays}d` : typeof stats.avgResolutionDays === 'number' ? `${stats.avgResolutionDays}d` : 'N/A'}
                      </span>
                    </div>
                    <p className="text-xs text-violet-500">{t('stats.avgProcessingDesc', { defaultValue: 'From submission to closure' })}</p>
                  </div>
                </div>
              )}
            </>
        ...
        {/* AI Insight Widgets — 7-Day Forecast + Duplicate Stats side by side */}
