...
import {
  categoryLabels,
  getCategoryLabel as getTranslatedCategory,
} from '@/lib/categories';
import {
  MapPin,
  Activity,
...
export default function HeatmapPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (user?.role === 'MUNICIPAL_AGENT' && user.municipality) {
        params.municipality =
          typeof user.municipality === 'string'
            ? user.municipality
            : user.municipality.name || user.municipality._id;
  ...
  const canAccess =
    user &&
    ['ADMIN', 'MUNICIPAL_AGENT', 'DEPARTMENT_MANAGER'].includes(user.role);
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    {data.length} hotspots •{' '}
                    {data.reduce((sum, p) => sum + p.count, 0)} total complaints
                  </p>
                </div>
              </div>
