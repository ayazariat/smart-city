...
export default function MunicipalitiesPage() {
  ...
  const [municipalityComplaints, setMunicipalityComplaints] = useState<ComplaintSummary[]>([]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-bold text-slate-800">Municipalities Directory</h1>
                <p className="text-xs text-slate-500">24 governorates · Public transparency</p>
              </div>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        {error ? (
          ...
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-slate-800 mb-2">No municipalities found</h3>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  ...
                  <div className="text-center p-4 bg-blue-50 rounded-xl">
                    <p className="text-2xl font-bold text-blue-700">
                      {Math.round(
                        (municipalities.reduce((s, m) => s + m.resolved, 0) / 
                         Math.max(municipalities.reduce((s, m) => s + m.total, 0), 1)) * 100
                      )}%
                    </p>
                    <p className="text-sm text-blue-600 mt-1">Avg Resolution Rate</p>
                  </div>
            ...
            {viewMode === "grid" ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredMunicipalities.map((mun) => {
                  const gradientClass = GRADIENT_CLASSES[mun.name] || "from-slate-900 to-slate-800";
                  const badgeClass = getPerformanceBadgeClass(mun.resolutionRate);
                    ...
                    >
                      ...
                      <div className="relative h-48 overflow-hidden">
                        ...
                        <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between">
                          <div>
                            <h3 className="text-xl font-bold text-white drop-shadow-lg">{mun.name}</h3>
                            <p className="text-white/80 text-sm">{mun.governorate}</p>
                          </div>
                          <div className="flex items-center gap-1 text-white/90 bg-black/30 backdrop-blur-sm px-3 py-1 rounded-full">
                            <MapPin className="w-4 h-4" />
                            <span className="text-sm font-medium">{mun.total}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="p-5">
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-4">
                          <div className="text-center p-3 bg-slate-50 rounded-xl">
                            <p className="text-lg font-bold text-slate-800">{mun.total}</p>
                            <p className="text-xs text-slate-500">Total</p>
                          </div>
                          <div className="text-center p-3 bg-green-50 rounded-xl">
                            <p className="text-lg font-bold text-green-700">{mun.resolved}</p>
                            <p className="text-xs text-green-600">Resolved</p>
                          </div>
                          <div className="text-center p-3 bg-blue-50 rounded-xl">
                            <p className="text-lg font-bold text-blue-700">{mun.resolutionRate}%</p>
                          ...
                          <div className="mb-4">
                            <span className={`px-3 py-1.5 rounded-full text-xs font-medium ${badgeClass}`}>
                              {mun.resolutionRate >= 70
                                ...
                                : "Needs Improvement"}
                              <span className="ml-1">- {mun.resolutionRate}% resolved</span>
                            </span>
                          </div>
                        )}
                        ...
                        {/* Description */}
                        {mun.description && (
                          <p className="text-sm text-slate-600 mb-4 line-clamp-2">{mun.description}</p>
                        )}
                      ...
                      <div className="flex items-start gap-6">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-4">
                            <div>
                              <h3 className="text-xl font-bold text-slate-800">{mun.name}</h3>
                              <p className="text-slate-500">{mun.governorate}</p>
                            </div>
                          ...
                          {mun.total > 0 ? (
                            <div className="flex items-center gap-6 mt-3">
                              <div className="flex items-center gap-2">
                                <span className="text-2xl font-bold text-slate-800">{mun.total}</span>
                                <span className="text-sm text-slate-500">total complaints</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <span className="text-2xl font-bold text-green-600">{mun.resolved}</span>
                                <span className="text-sm text-green-500">resolved</span>
                              </div>
                            </div>
                          ) : (
                            <p className="text-sm text-slate-500 mt-2">No complaints on record</p>
                          )}

                          {mun.description && (
                            <p className="text-sm text-slate-600 mt-3 line-clamp-2">{mun.description}</p>
                          )}
                        </div>
            ...
            <div className="bg-white rounded-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  <div className="text-center p-4 bg-slate-50 rounded-xl">
                    <p className="text-3xl font-bold text-slate-800">{selectedMunicipality.total}</p>
                    <p className="text-sm text-slate-500">Total Complaints</p>
                  </div>
                  <div className="text-center p-4 bg-green-50 rounded-xl">
                    <p className="text-3xl font-bold text-green-700">{selectedMunicipality.resolved}</p>
                    <p className="text-sm text-green-600">Resolved</p>
                  </div>
                  <div className="text-center p-4 bg-blue-50 rounded-xl">
                    <p className="text-3xl font-bold text-blue-700">{selectedMunicipality.resolutionRate}%</p>
                ...
                <div>
                  ...
                  {municipalityComplaints.length > 0 ? (
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                      {municipalityComplaints.slice(0, 20).map((complaint) => (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="font-medium text-slate-800 truncate">{complaint.title}</p>
                            <p className="text-sm text-slate-500">
                              {new Date(complaint.createdAt).toLocaleDateString()}
                            </p>
                          </div>
                          <span className={`px-2 py-1 rounded-lg text-xs font-medium whitespace-nowrap ${
                            complaint.status === "VALIDATED" ? "bg-blue-100 text-blue-700" :
                            ...
                            complaint.status === "CLOSED" ? "bg-slate-100 text-slate-700" :
                            complaint.status === "SUBMITTED" ? "bg-amber-100 text-amber-700" :
                            complaint.status === "REJECTED" ? "bg-red-100 text-red-700" :
                            "bg-slate-100 text-slate-700"
                          }`}>
                            {complaint.status === "VALIDATED" ? "Verified" :
                    ...
                    <div className="text-center py-8 bg-slate-50 rounded-xl">
                      ...
                      <p className="text-slate-500">No complaints recorded for this municipality</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-200 rounded-b-2xl flex gap-3 justify-end">
                 <Link
                   href={"/transparency/complaints?municipality=" + encodeURIComponent(selectedMunicipality.name)}
