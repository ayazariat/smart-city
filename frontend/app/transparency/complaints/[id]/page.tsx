...
export default function PublicComplaintDetailPage() {
    ...
    const fetchComplaint = async () => {
      try {
        ...
        const response = await fetch(
          `${apiUrl}/public/complaints/${complaintId}`,
          {
            credentials: 'include',
  ...
  const fetchComments = async () => {
    try {
      ...
      const res = await fetch(
        `${apiUrl}/public/complaints/${complaintId}/comments`
      );
  ...
  const handleUpvote = async () => {
    ...
    // Clear action parameter after execution to prevent re-triggering
    ...
    try {
      ...
      if (data.success && complaint) {
        setComplaint({
          ...complaint,
          confirmationCount:
            data.confirmationCount ?? (complaint.confirmationCount || 0) + 1,
        });
      }
    } catch {
      /* silent */
  ...
  const prevImage = () => {
    if (complaint?.media && complaint.media.length > 0) {
      setCurrentImageIndex(
        (prev) => (prev - 1 + complaint.media!.length) % complaint.media!.length
      );
    }
  };
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 via-white to-slate-50">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-xl font-bold text-slate-800 mb-2">
            {t('publicComplaint.notFound')}
          </h2>
          <p className="text-slate-600 mb-6">
            {error || t('publicComplaint.notPublicYet')}
          </p>
          <button
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <div className="hidden sm:block">
                <h1 className="text-lg font-bold text-slate-800 leading-tight">
                  {t('publicComplaint.complaintDetail')}
                </h1>
                <p className="text-xs text-slate-500">
                  {complaint.referenceId
                    ? `#${complaint.referenceId}`
                    : getComplaintIdDisplay(complaint._id)}
                </p>
              </div>
            </div>
      ...
      >
        <div className="flex flex-col h-full">
              ...
              >
                ...
                <div>
                  <h2 className="text-sm font-bold text-slate-800 leading-tight">
                    {t('publicComplaint.smartCity')}
                  </h2>
                  <p className="text-[10px] text-slate-400 font-medium">
                    {t('publicComplaint.publicDashboard')}
                  </p>
                </div>
              </Link>
              <button
          ...
          <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
            ...
            <Link
              href="/transparency#governorates"
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-slate-600 hover:bg-slate-50 hover:text-slate-800 border-l-[3px] border-transparent pl-[9px] transition-all"
            >
                ...
                <div className="relative h-72 md:h-96 bg-slate-100">
                  {resolveMediaUrl(complaint.media[currentImageIndex]?.url) ? (
                    <img
                      src={
                        resolveMediaUrl(
                          complaint.media[currentImageIndex]?.url
                        ) || ''
                      }
                      alt={complaint.title}
                  ...
                  {complaint.media.length > 1 && (
                    <>
                      <button
                        onClick={prevImage}
                        className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/90 rounded-full flex items-center justify-center shadow-lg hover:bg-white"
                      >
                        <ChevronLeft className="w-5 h-5 text-slate-700" />
                      </button>
                      <button
                        onClick={nextImage}
                        className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/90 rounded-full flex items-center justify-center shadow-lg hover:bg-white"
                      >
            ...
            {(complaint.status === 'RESOLVED' ||
              ...
              <div className="bg-green-50 rounded-2xl border border-green-200 p-6">
                ...
                <p className="text-sm text-green-700 mb-2">
                  {t('publicComplaint.resolvedByTeam')}
                </p>
                {(complaint.resolutionNote || complaint.resolutionNotes) && (
                  <p className="text-sm text-green-800 bg-white rounded-lg p-3 border border-green-200 mb-3">
                    {complaint.resolutionNote || complaint.resolutionNotes}
                  </p>
                )}
                {complaint.resolvedAt && (
                  <p className="text-xs text-green-600">
                    {t('publicComplaint.resolvedOn', {
                      date: new Date(complaint.resolvedAt).toLocaleDateString(
                        locale,
                ...
                {(complaint.afterPhotos || complaint.proofPhotos)?.length >
                  0 && (
                  <div className="grid grid-cols-2 gap-2 mt-3">
                    {(complaint.afterPhotos || complaint.proofPhotos || []).map(
                      (photo, idx) => {
                        ...
                        return proofUrl ? (
                          ...
                          <div
                            key={idx}
                            className="w-full h-32 bg-slate-100 rounded-lg flex items-center justify-center"
                          >
              ...
              <h3 className="text-base font-semibold text-slate-800 mb-4 flex items-center gap-2">
                ...
                {comments.length > 0 && (
                  <span className="ml-auto text-xs font-normal text-slate-400">
                    {t('publicComplaint.commentCount', { n: comments.length })}
                  </span>
                )}
              </h3>
              {user && token ? (
                ...
                <div className="text-center py-4 bg-slate-50 rounded-xl mb-4">
                  <p className="text-sm text-slate-500">
                    ...
                    >
                      {t('publicComplaint.signIn')}
                    </button>{' '}
                    {t('publicComplaint.toAddComment')}
                  </p>
                </div>
              )}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-md p-5">
              <h3 className="text-sm font-semibold text-slate-800 mb-4">
                {t('publicComplaint.statusTimeline')}
              </h3>
                    ...
                    <div key={step} className="flex gap-3">
                      <div className="flex flex-col items-center">
                        ...
                        {idx < statusSteps.length - 1 && (
                          <div
                            className={`w-0.5 h-8 ${isCompleted && idx < currentStepIndex ? 'bg-green-500' : 'bg-slate-200'}`}
                          />
                        )}
                      </div>
                      <div className="pb-6">
                        <p
                          className={`text-sm font-medium ${isCompleted ? 'text-slate-800' : 'text-slate-400'}`}
                        >
            ...
            <div className="bg-white rounded-2xl shadow-md p-5">
              <h3 className="text-sm font-semibold text-slate-800 mb-4">
                {t('publicComplaint.communityEngagement')}
              </h3>
              <div className="grid grid-cols-3 gap-2 mb-4">
                ...
                <div className="text-center p-2 bg-green-50 rounded-xl">
                  ...
                  <p className="text-[10px] text-slate-500">
                    {t('publicComplaint.confirms')}
                  </p>
                </div>
                <div className="text-center p-2 bg-blue-50 rounded-xl">
                  <Eye className="w-4 h-4 text-blue-500 mx-auto mb-1" />
                  <p className="text-lg font-bold text-slate-800">
                    {complaint.viewsCount != null
                      ? complaint.viewsCount >= 1000
                        ? `${(complaint.viewsCount / 1000).toFixed(1)}K`
                        : complaint.viewsCount
                      : 0}
                  </p>
                  <p className="text-[10px] text-slate-500">
                    {t('publicComplaint.views')}
                  </p>
                </div>
              </div>
