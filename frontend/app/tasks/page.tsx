...
> = {
  ...
  RESOLVED: {
    labelKey: 'tasks.resolved',
    bgClass: 'bg-green-50',
    textClass: 'text-green-700',
    borderClass: 'border-green-200',
  },
  CLOSED: {
    labelKey: 'tasks.closed',
  ...
  const handleStartWork = async (id: string) => {
    ...
    try {
      ...
      const errorMsg =
        error?.message ||
        error?.data?.message ||
        'Failed to start work. Make sure the task is assigned to you.';
          ...
          {loading ? (
            ...
            <div className="grid gap-4">
              {filteredTasks.map((task, index) => {
                const status =
                  statusConfig[task.status] || statusConfig.ASSIGNED;
                const urgency = task.urgency
                  ? urgencyConfig[task.urgency]
                  : null;
                const id = task._id || '';
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2 mb-3">
                            ...
                            <span
                              className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold ${status.bgClass} ${status.textClass}`}
                            >
                              <span
                                className={`w-1.5 h-1.5 rounded-full ${status.textClass.replace('text-', 'bg-')} animate-pulse-soft`}
                              />
                              {t(status.labelKey)}
                            </span>
                            ...
                            {urgency && (
                              <span
                                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${urgency.bgClass} ${urgency.textClass}`}
                              >
                                {t(urgency.labelKey)}
                              </span>
                            )}
                            ...
                            {task.slaDeadline &&
                              ...
                              (() => {
                                const diff =
                                  new Date(task.slaDeadline).getTime() -
                                  Date.now();
                                ...
                                return (
                                  <span
                                    className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${hours < 48 ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'}`}
                                  >
                                    <Clock className="w-3 h-3" />
                                    {days > 0
                                      ? `${days}d ${h}h left`
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            ...
                            {task.media && task.media.length > 0 && (
                              <div className="flex items-center gap-2 text-xs text-slate-600 bg-slate-50 p-2 rounded-lg">
                                ...
                                <span>
                                  {getPhotoCount(task.media)} photo(s)
                                </span>
                              </div>
                            )}
                          </div>
            ...
            {selectedTask && (
              <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
                <p className="text-sm font-medium text-slate-700">
                  {selectedTask.title || selectedTask.description?.slice(0, 80)}
                </p>
                <p className="text-xs text-slate-500 mt-1">
                  #{selectedTask.referenceId || selectedTask._id?.slice(-6)}
                </p>
              </div>
            )}
            <div>
              ...
              />
              <p className="text-xs text-slate-500 mt-1">
                {resolveNote.length}/20 characters minimum
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">
              ...
              >
                ...
                >
                  ...
                  <p className="text-sm text-slate-500">
                    Click to upload proof photos
                  </p>
                  {proofPhotos.length > 0 && (
                    <p className="text-xs text-green-600 mt-2">
                      {proofPhotos.length} file(s) selected
                    </p>
                  )}
                </div>
              </div>
              ...
              {proofPhotos.length > 0 && (
                <div className="grid grid-cols-3 gap-2 mt-3">
                  {proofPhotos.map((file, i) => (
                    <div
                      key={i}
                      className="relative h-20 bg-slate-100 rounded-lg overflow-hidden"
                    >
                      <img
