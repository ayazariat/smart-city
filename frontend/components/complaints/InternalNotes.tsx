...
const InternalNotes: React.FC<InternalNotesProps> = ({
  ...
  const isStaff =
    !['CITIZEN', undefined, null].includes(userRole) && userRole !== '';
  ...
  const visibleNotes =
    userRole === 'CITIZEN' ? notes.filter((n) => n.type === 'PUBLIC') : notes;
      ...
      {canAdd && isStaff && (
        <div className="mt-4 space-y-3">
          <textarea
            ...
            maxLength={500}
          />
          <p className="text-xs text-slate-500">{t('notes.helperText')}</p>
