...
export default function MunicipalityMiniMap({
  points,
  municipality,
}: MunicipalityMiniMapProps) {
  ...
  const filteredPoints = (points || []).filter((p) => {
    ...
    if (filter === 'active')
      return (
        !p.status || !['RESOLVED', 'CLOSED', 'ARCHIVED'].includes(p.status)
      );
  ...
  // Status breakdown
  const activeCount = (points || [])
    .filter((p) => !['RESOLVED', 'CLOSED', 'ARCHIVED'].includes(p.status || ''))
    .reduce((s, p) => s + p.count, 0);
  const resolvedCount = (points || [])
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm font-bold">
                {municipality
                  ? `${municipality} — Activity Map`
                  : t('map.title')}
              </span>
            </div>
            <p className="text-[11px] text-white/80">
              {totalAll} complaint{totalAll !== 1 ? 's' : ''} across{' '}
        ...
        <div className="flex items-center gap-4 mt-3 pt-3 border-t border-white/20">
          ...
          <div className="flex items-center gap-1.5 text-[11px] ml-auto">
            ...
            <span className="font-bold">
              {filteredPoints.filter((p) => p.count >= maxCount * 0.75).length}
            </span>
          </div>
        </div>
      </div>
                ...
                <Popup closeButton={false}>
                  ...
                  >
                    ...
                    {point.title && (
                      ...
                      >
                        {point.title.length > 60
                          ? point.title.slice(0, 57) + '…'
                          : point.title}
                      </p>
                    )}
                    ...
                    >
                      ...
                      {point.createdAt && (
                        <span>{formatTimeAgo(point.createdAt)}</span>
                      )}
                    </div>
                  </div>
                </Popup>
