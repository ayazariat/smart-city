...
function formatRelativeTime(date: string, t: (key: string, opts?: Record<string, unknown>) => string): string {
  const diff = Date.now() - new Date(date).getTime();
  const minutes = Math.floor(diff / 60000);
  const hours = Math.floor(diff / 3600000);
  const days = Math.floor(diff / 86400000);
  if (minutes < 1) return t("time.justNow", { defaultValue: "Just now" });
  if (minutes < 60) return t("time.minutesAgo", { count: minutes, defaultValue: `${minutes}m ago` });
...
export default function NotificationsPanel({ maxItems = 8, role }: NotificationsPanelProps) {
        ...
        <div className="flex items-center gap-2">
          {unreadCount > 0 && (
            <button
              ...
              title={t("dashboard.markAllRead", { defaultValue: "Mark all as read" })}
            >
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 text-slate-400">
            ...
            <p className="text-sm font-medium text-slate-500">
              {t("dashboard.noNotifications", { defaultValue: "No notifications yet" })}
            </p>
            <p className="text-xs text-slate-400 mt-1">
              {role === "CITIZEN"
                ? t("dashboard.noNotifCitizen", {
                    defaultValue: "You'll be notified when your complaint status changes.",
                  })
                : t("dashboard.noNotifStaff", {
                    defaultValue: "New complaints and updates will appear here.",
                  })}
            </p>
          </div>
        ) : (
              ...
              >
                ...
                {!notif.isRead && (
                  <button
                    ...
                    title={t("dashboard.markRead", { defaultValue: "Mark as read" })}
                  >
