...
function FitBounds({ data }: { data: ComplaintMapPoint[] }) {
  ...
  useEffect(() => {
    ...
    const bounds = L.latLngBounds(data.map((p) => [p.location.lat, p.location.lng]));
...
export default function ManagerComplaintMap({ data = [], municipality, height = "400px" }: ManagerComplaintMapProps) {
  ...
  if (loading) {
    return (
      <div className="rounded-2xl border border-slate-200 shadow-sm bg-white flex items-center justify-center" style={{ height }}>
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-sm bg-white relative" style={{ height }}>
      <MapContainer
        center={center}
              ...
              <div className="min-w-[200px] p-1">
                <div className="flex items-center gap-2 mb-2">
                  <div
                    ...
                    style={{ backgroundColor: STATUS_COLORS[point.status] || "#6b7280" }}
                  />
                ...
                {point.referenceId && (
                  <p className="text-xs font-bold text-slate-900 mb-1">#{point.referenceId}</p>
                )}
      ...
      {data.length === 0 && (
        <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center z-[1000]">
          ...
          <p className="text-sm text-slate-500 font-medium">No complaint locations</p>
          <p className="text-xs text-slate-400 mt-1">Complaints with coordinates will appear here</p>
        </div>
      )}
    </div>
  );
