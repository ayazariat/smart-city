...
export default function MunicipalityOverview({
    ...
    const fetchData = async () => {
      try {
        ...
        const [muniRes, heatmapRes, complaintsRes] = await Promise.all([
          fetch(`${apiUrl}/public/stats/by-municipality?period=all`)
            .then((r) => r.json())
            .catch(() => null),
          heatmapService
            .getHeatmapData(
  ...
  const governorates = municipalities.reduce(
    (acc, m) => {
    ...
    {} as Record<
      string,
      { total: number; resolved: number; municipalities: number }
    >
  );
    ...
    <div className="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden">
      ...
      <div className="p-5">
        {loading ? (
          ...
          (municipalities.length === 0 && heatmapPoints.length === 0) ? (
          <div className="text-center py-8">
            <MapPin className="w-8 h-8 text-slate-300 mx-auto mb-2" />
            <p className="text-sm text-slate-500">
              {t('municipalityOverview.noData')}
            </p>
            <p className="text-xs text-slate-400 mt-1">
              {t('municipalityOverview.noDataHint')}
            </p>
          </div>
        ) : (
          <>
              ...
              <div className="mb-4 p-4 bg-gradient-to-br from-primary/5 to-primary/10 rounded-xl border border-primary/20">
                ...
                <div className="grid grid-cols-3 gap-3 mt-3">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[10px] text-slate-500 mt-0.5">
                      {t('municipalityOverview.resolution')}
                    </p>
                  </div>
                  <div className="text-center">
                    ...
                    <p className="text-[10px] text-slate-500 mt-0.5">
                      {t('municipalityOverview.avgDays')}
                    </p>
                  </div>
                </div>
              </div>
                          ...
                          <div key={gov} className="group">
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <div className="flex items-center gap-3 text-xs">
                                ...
                                <span
                                  className={`font-medium ${rate >= 50 ? 'text-green-600' : 'text-amber-600'}`}
                                >
                                  {rate}%
                                </span>
                              </div>
