...
const NotificationBell = () => {
  ...
  const formatRelativeTime = (dateString: string) => {
    ...
    if (diff < 3600000) return t("common.timeAgo.minutes", { n: Math.floor(diff / 60000) });
    if (diff < 86400000) return t("common.timeAgo.hours", { n: Math.floor(diff / 3600000) });
      ...
      {dropdownOpen && (
        <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-200 z-50 animate-in slide-in-from-top-2 duration-200 max-h-96 overflow-y-auto">
           <div className="p-4 border-b border-slate-200">
             ...
             <Link href="/notifications" className="block mt-2 text-xs text-primary hover:underline">
          ...
          <div className="divide-y divide-slate-100">
             {todayNotifs.length > 0 && (
               <div>
                 <h4 className="px-4 py-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('notifications.today')}</h4>
                 {todayNotifs.slice(0, 5).map((notif) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className={`p-2 rounded-lg bg-blue-100 flex-shrink-0 ${getNotifStyle(notif.type).color}`}>
                        <Bell className="w-4 h-4" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-slate-900 line-clamp-2">{notif.message || notif.title}</p>
             ...
             {earlierNotifs.length > 0 && (
               <div>
                 <h4 className="px-4 py-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">{t('notifications.earlier')}</h4>
