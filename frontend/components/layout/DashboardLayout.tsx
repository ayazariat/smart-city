...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const humanizeNotificationKey = useCallback((value: string) => {
    const notificationFallbacks: Record<string, string> = {
      ...
      'notification.status.validated.desc':
        'Your complaint has been validated and will be processed.',
      ...
      'notification.status.assigned.desc':
        'Your complaint has been assigned to a team.',
      'notification.status.inProgress': 'In progress',
      'notification.status.inProgress.desc':
      ...
      'notification.assignedToYou.desc':
              ...
              {loadingNotifs ? (
                ...
                <div className="flex flex-col items-center justify-center py-10 text-slate-400">
                  ...
                  <p className="text-[10px] text-slate-300 mt-1 text-center px-4">
                    You&apos;ll be notified when complaints are validated,
                    ...
                    return (
                      <div
                        ...
                        onClick={() => {
                          ...
                          const targetComplaintId =
                            notif.complaint?._id || notif.relatedId;
                      ...
                      >
                        <div className="flex items-start gap-2.5">
                          <div
                            className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${!notif.isRead ? 'bg-primary' : 'bg-transparent'}`}
                          />
                          <div className="flex-1 min-w-0">
                            <p
                              className={`text-xs leading-tight line-clamp-2 ${!notif.isRead ? 'font-semibold text-slate-800' : 'font-medium text-slate-700'}`}
                            >
                              {title}
                            </p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-secondary-50 to-primary/10">
      ...
      <header
        className={`md:hidden bg-white border-b border-slate-200 shadow-sm sticky top-0 z-30`}
      >
      ...
      <header
        className={`bg-white border-b border-slate-200 shadow-sm sticky top-0 z-30 hidden md:block ${isRTL ? 'mr-0 md:mr-[260px]' : 'ml-0 md:ml-[260px]'}`}
      >
