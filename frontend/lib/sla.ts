...
export function getSlaDeadline(
  ...
  const urgencyKey = (urgency?.toUpperCase() ||
    'MEDIUM') as keyof typeof categorySLA;
...
export function formatRemainingTime(remainingHours: number): {
  text: string;
  color: string;
} {
  if (remainingHours <= 0) {
  ...
  if (remainingHours < 24) {
    ...
    return {
      text: `${hours}h left`,
      color: hours < 12 ? 'text-red-600' : 'text-orange-600',
    };
  }
      ...
      return {
        ...
        label:
          remainingHours !== undefined
            ? `${Math.abs(Math.round(remainingHours / 24))}d overdue`
            : 'Overdue',
        icon: '🚨',
