...
/* ═══════════════════════════════════════════════════════════════
   ...
   Features:
     • 5-role switcher (Citizen/Agent/Tech/Manager/Admin)
...
const URG = {
  ...
  CRITICAL: { l:"Critical", c:"#B71C1C", bg:"#FFEBEE", sla:"8h"      },
};
const CATS = [
...
// ── MOCK DATA ──
const MOCK = [
  { id:"RC-A3F2E1", cat:"EAU",           status:"IN_PROGRESS", title:"Water main leak causing road flooding near central pharmacy", loc:"Beni Khiar, Nabeul",   score:85, date:"Mar 5",  urg:"HIGH",     lat:55, lng:48, dept:"Infrastructure" },
  { id:"RC-B9D4C7", cat:"ROUTES",        status:"SUBMITTED",   title:"Large pothole on Avenue Habib Bourguiba damaging vehicles",   loc:"Tunis Centre",         score:92, date:"Mar 4",  urg:"CRITICAL", lat:35, lng:68, dept:null           },
  { id:"RC-C1E8F5", cat:"ECLAIRAGE",     status:"VALIDATED",   title:"Street lamps non-functional for 5 days — safety hazard at night", loc:"La Marsa",          score:68, date:"Mar 3",  urg:"MEDIUM",   lat:72, lng:30, dept:null           },
  { id:"RC-D5A2B0", cat:"DECHETS",       status:"ASSIGNED",    title:"Overflowing garbage bins near central market spreading odors", loc:"Sfax Medina",         score:74, date:"Mar 3",  urg:"HIGH",     lat:28, lng:55, dept:"Sanitation"    },
  { id:"RC-E7F3D9", cat:"SECURITE",      status:"RESOLVED",    title:"Broken sidewalk tiles causing injuries to pedestrians",       loc:"Sousse Beach",         score:96, date:"Mar 1",  urg:"CRITICAL", lat:60, lng:80, dept:"Safety"        },
  { id:"RC-F2G8H1", cat:"BIENS_PUBLICS", status:"CLOSED",      title:"Damaged park benches and broken fountain in city garden",     loc:"Ariana Park",          score:45, date:"Feb 28", urg:"LOW",      lat:45, lng:20, dept:"Public Works"  },
  { id:"RC-G4K2M8", cat:"ROUTES",        status:"SUBMITTED",   title:"Traffic signal malfunction causing accidents at intersection", loc:"Sousse Centre",        score:88, date:"Mar 6",  urg:"HIGH",     lat:18, lng:42, dept:null           },
  { id:"RC-H9L1P3", cat:"EAU",           status:"IN_PROGRESS", title:"Sewage backup in residential area health emergency",          loc:"Monastir",             score:97, date:"Mar 7",  urg:"CRITICAL", lat:82, lng:65, dept:"Water"         },
];

const ROLES = [
  { id:"citizen",    label:"Citizen",    user:"Aya Zariat",     role:"Citizen",           muni:"Beni Khiar" },
  { id:"agent",      label:"Agent",      user:"Mhalla Saadia",  role:"Municipal Agent",   muni:"Nabeul"     },
  { id:"technician", label:"Technician", user:"Ahmed Khelifi",  role:"Technician",        muni:"Nabeul"     },
  { id:"manager",    label:"Manager",    user:"Karim Trabelsi", role:"Dept. Manager",     muni:"Nabeul"     },
  { id:"admin",      label:"Admin",      user:"System Admin",   role:"Administrator",     muni:"—"          },
];

// ── ANIMATED COUNTER ──
function AnimNum({ to, delay=0, prefix="", suffix="" }) {
...
// ── MAP COMPONENT ──
function CityMap({ complaints, onSelect, selected, height = 400 }) {
  const [mode, setMode] = useState("all");
  const [hovMk, setHovMk] = useState(null);
  ...
  const catColors = { ROUTES:"#1565C0", DECHETS:"#795548", EAU:"#0288D1", ECLAIRAGE:"#F9A825", SECURITE:"#B71C1C", BIENS_PUBLICS:"#2E7D32", AUTRE:"#546E7A" };
    ...
    <div className="map-container" style={{ height }}>
      <div className="map-inner" style={{ height: "100%" }}>
        ...
        {filtered.map((c, i) => {
          ...
          const col = catColors[c.cat] || "#546E7A";
          const isSelected = selected?.id === c.id;
          return (
            <div key={c.id}
              className="mk"
              style={{ left:`${c.lng}%`, top:`${c.lat}%`, zIndex: isSelected ? 20 : hovMk===c.id ? 15 : 5 }}
              onClick={() => onSelect(c)}
              onMouseEnter={() => setHovMk(c.id)}
            ...
            >
              ...
              <div className="mk-label">{c.id} — {c.urg}</div>
            </div>
          );
        })}
...
function DetailPanel({ complaint, open, onClose, role }) {
  if (!complaint) return null;
  const cfg = ST[complaint.status] || ST.SUBMITTED;
  const slaOk = complaint.urg === "LOW" || complaint.urg === "MEDIUM";

  const tl = [
    { s:"SUBMITTED",   date:"Mar 5, 09:14", actor:"Aya Zariat",   note:"Complaint submitted online" },
    { s:"VALIDATED",   date:"Mar 5, 11:30", actor:"Agent Mhalla", note:"Verified — valid urban issue" },
    ...( ["ASSIGNED","IN_PROGRESS","RESOLVED","CLOSED"].includes(complaint.status)
      ? [{ s:"ASSIGNED", date:"Mar 5, 14:00", actor:"Manager Karim", note:"Assigned to Infrastructure dept." }] : []),
    ...( ["IN_PROGRESS","RESOLVED","CLOSED"].includes(complaint.status)
      ? [{ s:"IN_PROGRESS", date:"Mar 6, 08:30", actor:"Tech. Ahmed", note:"On-site repair started" }] : []),
    ...( ["RESOLVED","CLOSED"].includes(complaint.status)
      ? [{ s:"RESOLVED", date:"Mar 7, 16:00", actor:"Tech. Ahmed", note:"Issue resolved. Proof photo uploaded." }] : []),
  ];
    ...
    <>
      ...
      <div className={`detail-panel ${open ? "open" : ""}`}>
        <div className="dp-hdr">
          ...
          <div style={{ flex:1 }}>
            <div style={{ fontFamily:"DM Mono,monospace", fontSize:11, color:"var(--txt3)", marginBottom:2 }}>{complaint.id}</div>
        ...
        <div className="dp-body">
          ...
          {["ASSIGNED","IN_PROGRESS"].includes(complaint.status) && (
            <div className="sla-bar-wrap card" style={{ marginBottom:12 }}>
              ...
              <div style={{ fontSize:10, color:"var(--txt3)", marginTop:4 }}>Deadline: Mar 10, 08:00</div>
            </div>
          )}
          ...
          <div className="card card-sm" style={{ marginBottom:12 }}>
            ...
            <div style={{ fontSize:13, color:"var(--txt2)", lineHeight:1.7 }}>
              Water leaking from the main pipe on Avenue Principale, causing flooding and road damage. The issue has worsened significantly after last week's rain. Several residents have been affected and the road is partially blocked.
            </div>
            <div style={{ marginTop:10, display:"flex", gap:6, flexWrap:"wrap" }}>
              {["leak","flooding","road damage","urgent","main pipe"].map(kw => (
                <span key={kw} className="badge" style={{ background:"var(--purbg)", color:"var(--purple)", borderColor:"var(--purbdr)", fontSize:9 }}>
                  {Ic.ai} {kw}
                </span>
              ))}
              <span style={{ fontSize:9, color:"var(--txt4)", alignSelf:"center" }}>AI-extracted</span>
            </div>
          </div>
          ...
          <div className="card card-sm" style={{ marginBottom:12 }}>
            ...
            <div style={{ height:90, borderRadius:8, background:"linear-gradient(135deg,#e8f4e8,#d4ecd4)", position:"relative", overflow:"hidden", border:"1px solid var(--bdr)", marginBottom:8 }}>
              ...
              <div style={{ position:"absolute",bottom:5,left:7,fontSize:9,color:"var(--txt2)",background:"rgba(255,255,255,.9)",padding:"2px 6px",borderRadius:4,fontFamily:"DM Mono,monospace" }}>36.8065, 10.1815</div>
            </div>
            <div style={{ fontSize:12, color:"var(--txt2)", fontWeight:500 }}>{complaint.loc}</div>
          </div>
          ...
          <div className="card card-sm" style={{ marginBottom:12 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:6 }}>
              {["rgba(21,101,192,.15)","rgba(0,200,83,.12)","rgba(230,81,0,.12)"].map((bg,i) => (
                <div key={i} style={{ aspectRatio:"1",borderRadius:8,background:bg,border:"1px solid var(--bdr)" }} />
              ))}
            </div>
          </div>
          ...
          {role !== "citizen" && (
            <div className="card card-sm" style={{ marginBottom:12 }}>
              ...
              <div className="note-item note-blocage">
                <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:5 }}>
                  <span style={{ color:"var(--orange)" }}>{Ic.warn}</span>
                  <span style={{ fontSize:10, fontWeight:800, color:"var(--orange)" }}>BLOCKER</span>
                </div>
                <div className="note-content">Specialized equipment needed. Requesting additional support from central services.</div>
                <div className="note-meta"><span>Tech. Ahmed</span><span>Mar 6, 14:30</span></div>
              </div>
              <div className="note-item note-normal">
                <div className="note-content">Confirmed with resident — leak started 3 days ago. Priority elevated.</div>
                <div className="note-meta"><span>Agent Mhalla</span><span>Mar 5, 11:45</span></div>
              </div>
        ...
        <div className="dp-footer">
          {role === "agent" && complaint.status === "SUBMITTED" && (
            <><button className="btn primary" style={{ flex:1, fontSize:12 }}>{Ic.check} Validate</button>
              <button className="btn danger" style={{ flex:1, fontSize:12 }}>{Ic.x} Reject</button>
              <button className="btn secondary" style={{ flex:1, fontSize:12 }}>Assign Dept</button></>
          )}
          {role === "agent" && complaint.status === "VALIDATED" && (
            <button className="btn primary" style={{ flex:1, fontSize:12 }}>Assign Department</button>
          )}
          {role === "manager" && complaint.status === "ASSIGNED" && (
          ...
          {role === "technician" && complaint.status === "ASSIGNED" && (
            <button className="btn primary" style={{ flex:1, fontSize:12 }}>Start Work</button>
          )}
          {role === "technician" && complaint.status === "IN_PROGRESS" && (
            <><button className="btn primary" style={{ flex:1, fontSize:12 }}>Mark Resolved</button>
              <button className="btn warn" style={{ flex:1, fontSize:12 }}>{Ic.warn} Flag Blocker</button></>
          )}
          {role === "agent" && complaint.status === "RESOLVED" && (
            <button className="btn primary" style={{ flex:1, fontSize:12 }}>Close Complaint</button>
          )}
          {!["agent","manager","technician","admin"].includes(role) && (
...
export default function App() {
  ...
  const [aiPredicted, setAiPredicted] = useState(null);
  ...
  const openComplaint = (c) => { setSelC(c); setPanelOpen(true); };
  ...
  const filtered = filter === "ALL" ? MOCK : MOCK.filter(c => c.status === filter);
  ...
  const PAGE_TITLE = { dashboard:"Dashboard", complaints:"Complaints", report:"Report an Issue", queue:"Validation Queue", tasks:"My Tasks", analytics:"Analytics", users:"User Management", archive:"Archives" };
    ...
    <div className="sidebar">
        ...
        <div className="role-sw">
          {ROLES.map(r => (
            <button key={r.id} className={`rc ${role===r.id?"on":"off"}`}
              style={role!==r.id?{background:"rgba(255,255,255,.07)",color:"rgba(255,255,255,.6)",borderColor:"rgba(255,255,255,.1)"}:{}}
              onClick={() => { setRole(r.id); setPage("dashboard"); }}>
              {r.label}
            </button>
          ))}
        </div>
      ...
      <div className="sb-nav">
        ...
        {NAV.map(([id, label, icon]) => (
          <button key={id} className={`sb-item ${page===id?"active":""}`} onClick={() => { setPage(id); setShowNotif(false); }}>
            <span className="sb-ic">{icon}</span>
            {label}
            {id==="queue" && <span className="sb-badge">3</span>}
            {id==="tasks" && <span className="sb-badge green">5</span>}
          </button>
        ))}
      </div>

      <div className="sb-footer">
        <button className="sb-logout">{Ic.logout} Sign Out</button>
      </div>
    </div>
  );
          ...
          {showNotif && (
            <div className="notif-panel">
              <div className="np-hdr">
                ...
                <button className="np-clear">Mark all read</button>
              </div>
              {[
                { msg:"RC-B9D4C7 submitted — Critical pothole, Avenue Bourguiba", time:"2 min ago", c:"var(--red)"    },
                { msg:"SLA Alert: RC-A3F2E1 expires in 4h — take action",          time:"15 min ago",c:"var(--orange)" },
                { msg:"RC-C1E8F5 assigned to you — Street lighting, La Marsa",    time:"1h ago",    c:"var(--green3)" },
                { msg:"RC-E7F3D9 resolved by Tech. Ahmed — proof photo attached",  time:"3h ago",    c:"var(--teal)"   },
              ].map((n,i) => (
                <div key={i} className="notif-item">
                  <div className="ni-dot" style={{ background:n.c }} />
                  <div><div className="ni-msg">{n.msg}</div><div className="ni-time">{n.time}</div></div>
                </div>
              ))}
            </div>
          )}
  ...
  const Dashboard = () => {
    const STATS = {
      citizen:    [{ n:3,   l:"My Reports",     c:"var(--blue)",    bc:"var(--blue)"   }, { n:1, l:"In Progress", c:"var(--orange)", bc:"var(--orange)" }, { n:1, l:"Resolved",    c:"var(--green3)", bc:"var(--green3)"}, { n:0, l:"Rejected",   c:"var(--red)",    bc:"var(--red)"   }],
      agent:      [{ n:8,   l:"Awaiting Review",c:"var(--orange)",  bc:"var(--orange)" }, { n:34,l:"This Week",   c:"var(--blue)",   bc:"var(--blue)"   }, { n:127,l:"Total Done", c:"var(--green3)", bc:"var(--green3)"}, { n:2, l:"SLA Overdue",c:"var(--red)",    bc:"var(--red)"   }],
      technician: [{ n:5,   l:"Active Tasks",   c:"var(--orange)",  bc:"var(--orange)" }, { n:2, l:"In Progress", c:"var(--blue)",   bc:"var(--blue)"   }, { n:3, l:"Done Today",  c:"var(--green3)", bc:"var(--green3)"}, { n:1, l:"SLA At Risk",c:"var(--red)",    bc:"var(--red)"   }],
      manager:    [{ n:89,  l:"Total Active",   c:"var(--blue)",    bc:"var(--blue)"   }, { n:74,l:"Resolution %",c:"var(--green3)", bc:"var(--green3)" }, { n:36,l:"Avg TMA (h)", c:"var(--orange)", bc:"var(--orange)"}, { n:4, l:"SLA Overdue",c:"var(--red)",    bc:"var(--red)"   }],
      admin:      [{ n:264, l:"Municipalities", c:"var(--purple)",  bc:"var(--purple)" }, { n:1842,l:"Total Complaints",c:"var(--blue)",bc:"var(--blue)"}, { n:73,l:"Resolution %",c:"var(--green3)", bc:"var(--green3)"}, { n:12,l:"Active Agents",c:"var(--orange)",bc:"var(--orange)"}],
    }[role];
    ...
    return (
      <div className="page">
        ...
        {/* Map overview (always visible on dashboard) */}
        <div style={{ marginBottom:20 }}>
          ...
          <CityMap complaints={MOCK} onSelect={openComplaint} selected={selC} height={280} />
        </div>
        ...
        <div className="card" style={{ padding:0, overflow:"hidden" }}>
          {MOCK.slice(0,4).map((c,i) => (
            <div key={c.id} className="cr au" style={{ animationDelay:`${200+i*55}ms` }} onClick={() => openComplaint(c)}>
              <div className="cr-left">
                <div className="cr-top">
                  <span className="cr-id">{c.id}</span>
                  <SBadge s={c.status} /><UBadge u={c.urg} />
                  {c.dept && <span className="badge" style={{ background:"var(--bg3)",color:"var(--txt3)",borderColor:"var(--bdr)",fontSize:9 }}>{c.dept}</span>}
                </div>
                <div className="cr-title">{c.title}</div>
                <div className="cr-meta">
                  <span className="cr-meta-ic">{Ic.pin} {c.loc}</span>
                  <span className="cr-meta-ic">{Ic.clock} {c.date}</span>
                </div>
              </div>
              <div style={{ flexShrink:0, textAlign:"right" }}>
                <div style={{ fontFamily:"DM Mono,monospace", fontSize:13, fontWeight:700, color: c.score>80?"var(--red)":c.score>60?"var(--orange)":"var(--green3)" }}>{c.score}</div>
                <div style={{ fontSize:9, color:"var(--txt4)", marginTop:2 }}>priority</div>
              </div>
            </div>
          ))}
        </div>
    ...
    <div className="page">
      {submitted && (
        <div className="alert" style={{ background:"var(--accentbg)", borderColor:"var(--accentbdr)" }}>
          ...
          <div>
            <div style={{ fontSize:13,fontWeight:800,color:"var(--green)" }}>Complaint submitted — RC-K7M2N9</div>
            <div style={{ fontSize:11,color:"var(--txt3)",marginTop:2 }}>You'll receive email updates as your complaint progresses through the workflow.</div>
          </div>
        </div>
      )}
      ...
      {view === "list" && (
        <div className="card" style={{ padding:0, overflow:"hidden" }}>
          {filtered.length === 0
            ...
            : filtered.map((c,i) => (
              <div key={c.id} className="cr sr" style={{ animationDelay:`${i*45}ms` }} onClick={() => openComplaint(c)}>
                <div className="cr-left">
                  <div className="cr-top">
                    <span className="cr-id">{c.id}</span>
                    <SBadge s={c.status} /><UBadge u={c.urg} />
                  ...
                  <div className="cr-meta">
                    <span className="cr-meta-ic">{Ic.pin} {c.loc}</span>
                    <span className="cr-meta-ic">{Ic.clock} {c.date}</span>
                    <span className="cr-meta-ic">{CATS.find(cat=>cat.id===c.cat)?.name}</span>
                  </div>
                  <div className="cr-score">
                    <div className="score-bar"><div className="score-fill" style={{ width:`${c.score}%`, background:c.score>80?"var(--red)":c.score>60?"var(--orange)":"var(--green3)" }} /></div>
                    <span style={{ fontFamily:"DM Mono,monospace",fontSize:10,color:"var(--txt3)",flexShrink:0 }}>Score {c.score}</span>
                  </div>
                </div>
              </div>
            ))
      ...
      {view === "split" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, height:560 }}>
          <div className="card" style={{ padding:0, overflow:"hidden auto", height:560 }}>
            {filtered.map((c,i) => (
              <div key={c.id} className="cr" style={{ background:selC?.id===c.id?"var(--accentbg)":undefined, borderLeft:selC?.id===c.id?"3px solid var(--green)":undefined }} onClick={() => setSelC(c)}>
                <div className="cr-left">
                  <div className="cr-top"><span className="cr-id">{c.id}</span><SBadge s={c.status} /><UBadge u={c.urg} /></div>
                  <div className="cr-title" style={{ fontSize:12 }}>{c.title}</div>
                  <div className="cr-meta">{Ic.pin} {c.loc}</div>
                </div>
              </div>
            ))}
          </div>
  ...
  const ReportForm = () => {
    ...
    const doSubmit = () => {
      setSubmitting(true);
      setTimeout(() => { setSubmitting(false); setSubmitted(true); setPage("complaints"); setStep(1); }, 1600);
    };
    return (
      <div className="page" style={{ maxWidth:680, margin:"0 auto" }}>
          ...
          <div className="form-section">
            ...
            <div className="map-picker" onClick={handleMapClick}>
              ...
              <svg style={{ position:"absolute",inset:0,width:"100%",height:"100%",opacity:.2 }}>
                <line x1="0" y1="45%" x2="100%" y2="43%" stroke="#1B5E20" strokeWidth="3" />
                <line x1="0" y1="70%" x2="100%" y2="68%" stroke="#1B5E20" strokeWidth="2" />
                <line x1="35%" y1="0" x2="37%" y2="100%" stroke="#1B5E20" strokeWidth="3" />
                <line x1="72%" y1="0" x2="70%" y2="100%" stroke="#1B5E20" strokeWidth="2" />
              </svg>
              {pinPos ? (
                <>
            ...
            <div className="grid2" style={{ gap:8 }}>
              <button className="btn secondary" style={{ fontSize:12 }}>{Ic.pin} Use My GPS</button>
              <button className="btn secondary" style={{ fontSize:12 }}>{Ic.map} Pick on Map</button>
            </div>
          </div>
        </div>}
        ...
        {step===2 && <div className="au">
          <div className="form-section" style={{ marginBottom:12 }}>
            ...
            <div style={{ fontSize:10,color:"var(--txt4)",textAlign:"right",marginTop:3,fontFamily:"DM Mono,monospace" }}>{form.title.length}/150</div>
          </div>
          <div className="form-section" style={{ marginBottom:12 }}>
            ...
            <div className="cat-grid">
              {CATS.map(c => (
                <button key={c.id} className={`cat-card ${form.cat===c.id?"selected":""}`} onClick={()=>setForm(f=>({...f,cat:c.id,categoryManuallySet:true}))}>
          ...
          <div className="form-section" style={{ marginBottom:12 }}>
            ...
            <div className="urg-row">
              {[["LOW","#43A047","#E8F5E9","7d"],["MEDIUM","#F57F17","#FFFDE7","5d"],["HIGH","#E65100","#FFF3E0","48h"],["CRITICAL","#B71C1C","#FFEBEE","8h"]].map(([k,c,bg,sla],i)=>(
                <button key={k} className={`urg-item ${form.urg===i+1?"selected":""}`}
                  ...
                  <div className="urg-lbl" style={{ color:form.urg===i+1?c:"var(--txt2)" }}>{URG[k].l}</div>
          ...
          <div className="form-section">
            ...
            <div style={{ fontSize:10,color:form.desc.length>=20?"var(--green3)":"var(--txt4)",textAlign:"right",marginTop:3,fontFamily:"DM Mono,monospace" }}>{form.desc.length}/1000 {form.desc.length>=20?"✓":""}</div>
          </div>
        </div>}
        ...
        {step===3 && <div className="au">
          <div className="form-section">
            ...
            <div className="thumb-grid">
              {["rgba(21,101,192,.12)","rgba(0,200,83,.1)","rgba(230,81,0,.1)"].map((bg,i)=>(
                <div key={i} className="thumb" style={{ background:bg }}>
                  <button className="thumb-rm">×</button>
                </div>
              ))}
              <div style={{ aspectRatio:"1",borderRadius:"var(--rsm)",border:"1.5px dashed var(--bdr2)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"all .15s" }}>
                <span style={{ fontSize:20,color:"var(--txt4)" }}>+</span>
              </div>
            </div>
          </div>
        </div>}
        ...
        {step===4 && <div className="au">
          <div className="form-section">
            ...
            {[
              ["Location",    `${(36.8065+pinPos.y*0.01).toFixed(4)}, ${(10.1815+pinPos.x*0.01).toFixed(4)} — Nabeul`],
  ...
  const AgentQueue = () => (
    <div className="page">
      <div className="grid4" style={{ marginBottom:18 }}>
        {[{ n:3,l:"Pending Validation",c:"var(--orange)" },{ n:34,l:"Validated Today",c:"var(--green3)" },{ n:2,l:"SLA Overdue",c:"var(--red)" },{ n:12,l:"Awaiting Dept.",c:"var(--purple)" }].map((s,i)=>(
      ...
      {MOCK.filter(c=>["SUBMITTED","VALIDATED"].includes(c.status)).map((c,i)=>(
        <div key={c.id} className={`qi ${c.urg==="CRITICAL"?"urgent":c.urg==="HIGH"?"high":"medium"} au`} style={{ animationDelay:`${i*60}ms` }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7 }}>
            <div>
              <div style={{ display:"flex",alignItems:"center",gap:7,marginBottom:4,flexWrap:"wrap" }}>
                <span style={{ fontFamily:"DM Mono,monospace",fontSize:11,color:"var(--txt3)" }}>{c.id}</span>
                <SBadge s={c.status} /><UBadge u={c.urg} />
              </div>
              ...
              <div style={{ fontSize:11,color:"var(--txt3)",display:"flex",gap:10 }}>
                <span>{Ic.pin} {c.loc}</span><span>{Ic.clock} {c.date}</span>
              </div>
            </div>
            <div style={{ fontFamily:"DM Mono,monospace",fontSize:16,fontWeight:800,color:c.score>80?"var(--red)":"var(--orange)",flexShrink:0 }}>{c.score}</div>
          </div>
          <div style={{ display:"flex",gap:7 }}>
            <button className="btn primary sm" style={{ flex:1 }}>{Ic.check} Validate</button>
            <button className="btn danger sm" style={{ flex:1 }}>{Ic.x} Reject</button>
  ...
  const TechTasks = () => (
    <div className="page">
      <div className="grid3" style={{ marginBottom:18 }}>
        {[{ n:5,l:"Active Tasks",c:"var(--orange)" },{ n:3,l:"Done Today",c:"var(--green3)" },{ n:1,l:"SLA At Risk",c:"var(--red)" }].map((s,i)=>(
          <div key={i} className="stat-card au" style={{ animationDelay:`${i*50}ms`,borderTopColor:s.c }}>
            <div className="stat-n" style={{ color:s.c }}><AnimNum to={s.n} delay={i*55} /></div>
            <div className="stat-l">{s.l}</div>
          </div>
      ...
      {MOCK.filter(c=>["ASSIGNED","IN_PROGRESS"].includes(c.status)).map((c,i)=>{
        const slaOk = c.urg!=="CRITICAL";
        return (
          <div key={c.id} className="task sr" style={{ animationDelay:`${i*65}ms`, borderLeft:`3px solid ${slaOk?"var(--orange)":"var(--red)"}` }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8 }}>
              <div>
                ...
                <div style={{ fontSize:11,color:"var(--txt3)" }}>{Ic.pin} {c.loc}</div>
              </div>
              {/* SLA countdown */}
              <div style={{ textAlign:"right",flexShrink:0 }}>
                ...
                <div style={{ marginTop:5, width:60, height:4, background:"var(--bg3)", borderRadius:4, overflow:"hidden" }}>
                  <div style={{ width:`${slaOk?35:88}%`,height:"100%",borderRadius:4,background:slaOk?"var(--orange)":"var(--red)" }} />
                </div>
              </div>
            </div>
            <div style={{ display:"flex",gap:7 }}>
              {c.status==="ASSIGNED"    && <button className="btn primary sm" style={{ flex:1 }}>Start Work</button>}
              {c.status==="IN_PROGRESS" && <button className="btn primary sm" style={{ flex:1 }}>Mark Resolved</button>}
  ...
  const Analytics = () => {
    ...
    return (
      <div className="page">
        <div className="grid4" style={{ marginBottom:18 }}>
          {[{ n:89,l:"Total Active",c:"var(--blue)" },{ n:74,l:"Resolution %",c:"var(--green3)" },{ n:36,l:"Avg TMA (h)",c:"var(--orange)" },{ n:4,l:"SLA Overdue",c:"var(--red)" }].map((s,i)=>(
            <div key={i} className="stat-card au" style={{ animationDelay:`${i*50}ms`,borderTopColor:s.c }}>
              <div className="stat-n" style={{ color:s.c }}><AnimNum to={s.n} delay={80+i*60} /></div>
              <div className="stat-l">{s.l}</div>
            </div>
        ...
        <div className="card au" style={{ animationDelay:"260ms", marginBottom:16 }}>
          ...
          <CityMap complaints={MOCK} onSelect={openComplaint} selected={selC} height={240} />
        </div>
        <div style={{ display:"flex",gap:10 }}>
  ...
  // ═══ ARCHIVE PAGE ═══
  const Archive = () => {
    const archived = MOCK.filter(c => ["CLOSED","REJECTED"].includes(c.status));
    return (
      <div className="page">
        ...
        <div className="filter-row">
          {["all","closed","rejected"].map(f=>(
            <button key={f} className={`fchip ${archiveFilter===f?"on":""}`} onClick={()=>setArchiveFilter(f)} style={{ textTransform:"capitalize" }}>{f==="all"?"All Archives":f.charAt(0).toUpperCase()+f.slice(1)}</button>
          ))}
        </div>
        <div className="card" style={{ padding:0,overflow:"hidden" }}>
          {archived.filter(c=>archiveFilter==="all"||c.status.toLowerCase()===archiveFilter).map((c,i)=>(
            <div key={c.id} className="cr" style={{ opacity:.85 }} onClick={()=>openComplaint(c)}>
              <div className="cr-left">
                <div className="cr-top">
                  <span className="cr-id">{c.id}</span>
                  <SBadge s={c.status} />
                ...
                <div className="cr-title">{c.title}</div>
                <div className="cr-meta"><span>{Ic.pin} {c.loc}</span><span>{Ic.clock} {c.date}</span></div>
              </div>
            </div>
          ))}
          {archived.filter(c=>archiveFilter==="all"||c.status.toLowerCase()===archiveFilter).length===0 && (
  ...
  const PAGES = { dashboard:<Dashboard/>, complaints:<Complaints/>, report:<ReportForm/>, queue:<AgentQueue/>, tasks:<TechTasks/>, analytics:<Analytics/>, users:<Users/>, archive:<Archive/> };
  ...
  return (
    <div>
      ...
      <DetailPanel complaint={selC} open={panelOpen} onClose={closePanel} role={role} />
    </div>
  );
}
